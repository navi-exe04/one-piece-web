...
<template>
  <section class="max-w-200 mx-auto">
    ...
    <div
      v-if="isLoading"
      class="font-body text-2xl text-wanted-paper text-center animate-pulse mt-8"
    >
    ...
    >
      <div className="w-1/3 border-print-letter object-cover">
        <img
          :src="store.getCharacterImage(selectedCharacter.name) || OnePieceLogo"
      ...
      <div className="w-2/3 flex flex-col justify-center">
        <div class="text-center mb-6">
          ...
          <div
            class="flex content-between font-body text-xl text-print-letter gap-4 justify-center mt-4"
          >
            <span v-if="selectedCharacter.size !== ''"
              ><strong>Size:</strong> {{ selectedCharacter.size }}</span
            >
            <span v-if="selectedCharacter.age !== ''"
              ><strong>Age:</strong> {{ selectedCharacter.age }}</span
            >
            <span v-if="selectedCharacter.status !== ''"
              ><strong>Status:</strong> {{ selectedCharacter.status }}</span
            >
          </div>
        </div>
